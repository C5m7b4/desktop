const IMAGES = {
  image1: new URL("./test.png", import.meta.url).href,
  orb: new URL("./orb1.png", import.meta.url).href,
  tree: new URL("./tree1.png", import.meta.url).href,
  pivot: new URL("./pivot.png", import.meta.url).href,
  pivot1: new URL("./pivot1.png", import.meta.url).href,
  calc: new URL("./calc.png", import.meta.url).href,
  chart1: new URL("./charts1.png", import.meta.url).href,
  grid: new URL("./grid.png", import.meta.url).href,
  barchart: new URL("./barchart.png", import.meta.url).href,
  boxplot: new URL("./boxplot.png", import.meta.url).href,
};

export default IMAGES;
